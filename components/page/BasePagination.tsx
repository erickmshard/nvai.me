...
function LinkTag({
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        'flex-center size-8 rounded-[4px] border border-transparent bg-transparent text-black hover:cursor-pointer hover:bg-gray-300 hover:text-black',
        className,
      )}
    >
      {children}
  ...
  if (type === 'page') {
    return (
      <LinkTag
        href={route}
        title={page.toString()}
        className={page === currentPage ? 'border-black bg-[#e0e0e0] text-black' : ''}
      >
        {page}
      </LinkTag>
    );
