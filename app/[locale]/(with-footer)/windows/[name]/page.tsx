...
export async function generateMetadata({ params: { name } }: { params: { name: string } }): Promise<Metadata> {
  ...
  const { data } = await supabase.from('windows_app').select('title,content,icon_url').eq('name', name).maybeSingle();
  if (!data) return {};
  const baseTitle = `${data.title} Download for Windows (Latest) | Nav ai`;
  const descRaw = `${data.content ? `${data.content} ` : ''}Download the latest version of ${data.title} for Windows for free. Safe direct download via Nav ai.`;
  const description = descRaw.length > 160 ? `${descRaw.slice(0, 157)}...` : descRaw;
  const image = data.icon_url || undefined;
  return {
    title: baseTitle,
    description,
    alternates: { canonical: `/windows/${encodeURIComponent(name)}` },
    openGraph: {
      type: 'website',
      url: `/windows/${encodeURIComponent(name)}`,
      title: baseTitle,
      description,
...
export default async function Page({ params: { name } }: { params: { name: string } }) {
  const supabase = createPublicClient(1800);
  const { data } = await supabase.from('windows_app').select('*').eq('name', name).maybeSingle();
  if (!data) {
    notFound();
  }
  ...
  const ldJson = {
    ...
    offers: undefined,
    downloadUrl: `${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/dl?name=${encodeURIComponent(data.name)}`,
  ...
  return (
    <div className='relative w-full bg-gray-50 text-black'>
      <div className='mx-auto w-full max-w-pc px-3 py-6 lg:px-0 lg:py-10'>
        ...
        <div className='rounded-2xl border border-gray-200 bg-white p-4 lg:p-6'>
          <div className='flex flex-col items-start gap-4 lg:flex-row lg:items-center lg:justify-between'>
            ...
            <a
              href={`/api/dl?name=${encodeURIComponent(data.name)}`}
              target='_blank'
      ...
      {/* Latest in Windows */}
      <Suspense>
        <LatestAppsGrid platform='windows' excludeName={data.name} moreHref='/windows' title='Latest Windows Apps' />
      </Suspense>
    </div>
  );
}
