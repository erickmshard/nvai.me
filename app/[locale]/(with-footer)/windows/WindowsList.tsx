...
import type { WindowsApp } from '@/db/supabase/types';

import SearchForm from '@/components/home/SearchForm';
import BasePagination from '@/components/page/BasePagination';

const WINDOWS_PAGE_SIZE = 24;

export default async function WindowsList({ pageNum, locale }: { pageNum?: string; locale?: string }) {
  const supabase = createClient();
  const currentPage = pageNum ? Number(pageNum) : 1;
  const start = (currentPage - 1) * WINDOWS_PAGE_SIZE;
  const end = start + WINDOWS_PAGE_SIZE - 1;
  ...
  const list = (data || []) as WindowsApp[];

  const site = process.env.NEXT_PUBLIC_SITE_URL || '';
  const langPrefix = locale && locale !== 'en' ? `/${locale}` : '';
  const pagePath = pageNum ? `/windows/page/${encodeURIComponent(pageNum)}` : '/windows';

  const ldJson = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'Windows Software',
    description: 'Discover curated Windows apps with safe, direct downloads. Get the latest versions free on Nav ai.',
    ...
    mainEntity: {
      ...
      itemListElement: list.map((item, i) => ({
        '@type': 'ListItem',
        position: start + i + 1,
        url: site ? `${site}${langPrefix}/windows/${encodeURIComponent(item.name)}` : undefined,
        name: item.title,
  ...
  return (
    <div className='mx-auto w-full max-w-pc px-3 pb-16 lg:pb-24'>
      ...
      <div className='my-5 flex flex-col gap-1 text-center lg:my-10 lg:gap-3'>
        <h1 className='text-2xl lg:text-5xl'>Windows Apps</h1>
        <h2 className='text-xs text-black/70 lg:text-sm'>Curated Windows software you can download safely.</h2>
      </div>
