...
export default async function MacList({ pageNum, locale }: { pageNum?: string; locale?: string }) {
  const supabase = createClient();
  const currentPage = pageNum ? Number(pageNum) : 1;
  const start = (currentPage - 1) * MAC_PAGE_SIZE;
  const end = start + MAC_PAGE_SIZE - 1;
  ...
  const list = (data || []) as MacApp[];

  const site = process.env.NEXT_PUBLIC_SITE_URL || '';
  const langPrefix = locale && locale !== 'en' ? `/${locale}` : '';
  const pagePath = pageNum ? `/mac/page/${encodeURIComponent(pageNum)}` : '/mac';

  const ldJson = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'Mac Software',
    description: 'Discover curated Mac apps with safe, direct downloads. Get the latest versions free on Nav ai.',
    ...
    mainEntity: {
      ...
      itemListElement: list.map((item, i) => ({
        '@type': 'ListItem',
        position: start + i + 1,
        url: site ? `${site}${langPrefix}/mac/${encodeURIComponent(item.name)}` : undefined,
        name: item.title,
